...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0/dist/js/bootstrap.bundle.min.js"
    ...
    function checkname() {
       var name=document.getElementById("name").nodeValue;
        var erroname = document.getElementById("errorname");
        var regexName = /^([\\p{Lu}][\\p{Ll}]{1,8})(\\s([\\p{Lu}]|[\\p{Lu}][\\p{Ll}]{1,10})){0,5}$/;
        if (name === "" || name == null) {
            erroname.innerHTML = "Họ tên không để trống";
        } else if (!regexName.test(name)) {
            erroname.innerHTML = "Tên không hợp lệ";
        } else {
            erroname.innerHTML = "";
            return name
        }
